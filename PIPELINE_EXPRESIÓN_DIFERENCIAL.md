...
# Copiar archivos de hombres a otro directorio (para ello necesitamos una rchivo como "data/raw/men_id_newrnaseq.txt" con las muestras de hombres)
while read identifier; do
  find data/processed/cuantificacion/ -type f -name "${identifier}_.isoforms.results" -exec cp {} data/processed/cuantificacion/man/ \;
done < data/raw/men_id_newrnaseq.txt

# Establecer directorio
setwd("data/processed/cuantificacion/hombre")

# Cargar archivos y establecer rutas
dir <- "data/processed/cuantificacion/hombre"
samples <- read.table("data/raw/metadata_new_rnaseq_sinbajacalidad.txt", header = TRUE)

# Cambiar formato de datos a binario y filtrar mujeres
samples$Gender <- ifelse(samples$Gender == "Female", 1, ifelse(samples$Gender == "Male", 0, samples$Gender))
samples<-samples[samples$Gender==0, ]
...
# 4.Generar heat map  
png("data/processed/Deseq2/men/sample_distance_men_heatmap.png", width = 800, height = 600)  
pheatmap(sampleDistMatrix, 
         clustering_distance_rows=sampleDists, 
         clustering_distance_cols=sampleDists, 
         col=colors)
...
# 3. Generar gráfica de pca 
png("data/processed/Deseq2/men/pca_plot_men_per_sample.png", width = 800, height = 600)  
ggplot(pcaData, aes(PC1, PC2, color=Status, shape=Ethnicity)) +
  geom_point(size=3) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) + 
...
# Graficar el número de genes aberrantes por muestra
png("data/processed/Deseq2/men/aberrant_genes_men_per_sample.png", width = 800, height = 600)
ggplot(df_aberrant, aes(x = reorder(Sample, -Aberrations), y = Aberrations)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(
    title = "Número de genes aberrantes por muestra",
...
# Graficar la dispersión de los datos
png("data/processed/Deseq2/men/postfiltering_plotDispEsts_men_rnaseq_with_IMC.png", width = 800, height = 600)
plotDispEsts(dds_2)
dev.off()
...
# Guardar resultados significativos
write.table(significativos, "data/processed/Deseq2/men/DE_genes_0.05_men.txt", quote=F, row.names=T)
...
par(mar=c(8,5,2,2))
png("data/processed/Deseq2/men/cooks_distance_sample_outlier_detection.png", width = 800, height = 600)
boxplot(log10(assays(dds_2)[["cooks"]]), range=0, las=2)
dev.off()
...
# Graficar el MA plot 
png("data/processed/Deseq2/men/plotMA_rnaseq_men.png", width = 800, height = 600)
ggplot(ma_data, aes(x = log10(baseMean), y = log2FoldChange, color = over_infra)) +
  geom_point(alpha = 0.5) +  # Puntos semi-transparentes
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +  # Línea en log2FC = 0
  scale_color_manual(values = c("black", "blue")) +  # No significativo: negro, significativo: azul
...
labels=res$symbol
png("data/processed/Deseq2/men/enhanced_volcano_plot_rnaseq_men.png", width = 800, height = 600)
EnhancedVolcano(res,
    lab = labels,
    x = 'log2FoldChange',
    y = 'pvalue',
title = 'Men volcano plot: Control vs Asthma',
pCutoff = 10e-5,
    FCcutoff = 0.5)
dev.off()
...
# Gráfico de puntos
png("data/processed/Deseq2/men/KEGG_analysis_men_0.05.png", width = 800, height = 600)
dotplot(KEGG_genes, showCategory=20)
dev.off()

# Crear Emmaplot de enriquecimiento KEGG
KEGG_genes_sim <- pairwise_termsim(KEGG_genes)
png("data/processed/Deseq2/men/emapplot_routes_enrichment_analysis_men_0.05.png", width = 800, height = 600)
emapplot(KEGG_genes_sim, showCategory = 20)
dev.off()
...
# Crear gráfica con los resultados significativos
png("data/processed/Deseq2/men/GO_analysis_men_0.05.png", width = 800, height = 600)
dotplot(enrich_go, showCategory = 20)
dev.off()
...
# Crear Emmaplot de enriquecimiento GO
enrich_go_sim <- pairwise_termsim(enrich_go)
png("data/processed/Deseq2/men/GO_emmaplot_analysis_men_0.05.png", width = 800, height = 600)
emapplot(enrich_go_sim, showCategory = 20)
dev.off()
```
